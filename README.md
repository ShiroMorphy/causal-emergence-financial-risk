...
The authoritative numerical results are generated by the deterministic 12-restart/100-iteration pipeline. Each canonical series, null-inference run, and checkpoint records the estimator specification and exact source hash. The current estimates and inference tables are stored under `reports/final_submission_source_of_truth/` and `reports/tables/`; hard-coded legacy findings are intentionally not duplicated here.
