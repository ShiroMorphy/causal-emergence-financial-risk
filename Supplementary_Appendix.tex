...
\textbf{Calm Period (2005)} 
 & $H_0^{\text{circ}}$ (Circular) & 999 & 0.8387 & +0.2207 & +0.4325 & +4.95 & 0.0010 & -- \\
 & $H_0^{\text{diag}}$ (VAR Diag) & 999 & 0.8387 & +0.0717 & +0.1201 & +29.33 & 0.0010 & -- \\
 & $H_0^{\text{static}}$ (Static Mode) & 9,999 & 0.8387 & +0.6904 & +0.7983 & +2.48 & 0.0254 & 0.0396 \\
 & $H_0^{\text{diag+contemp}}$ (Cross-lag Isol.) & 9,999 & 0.8387 & +0.6952 & +0.7837 & +2.70 & 0.0132 & 0.0396 \\
\midrule
\textbf{2008 GFC Peak} 
 & $H_0^{\text{circ}}$ (Circular) & 999 & 1.8131 & +0.3734 & +0.6261 & +7.78 & 0.0010 & -- \\
 & $H_0^{\text{diag}}$ (VAR Diag) & 999 & 1.8131 & +0.2039 & +0.2711 & +40.47 & 0.0010 & -- \\
 & $H_0^{\text{static}}$ (Static Mode) & 9,999 & 1.8131 & +1.2066 & +1.5099 & +3.81 & 0.0010 & 0.0040 \\
 & $H_0^{\text{diag+contemp}}$ (Cross-lag Isol.) & 9,999 & 1.8131 & +1.0370 & +1.1451 & +12.21 & 0.0001 & 0.0006 \\
\midrule
\textbf{2020 COVID Shock} 
 & $H_0^{\text{circ}}$ (Circular) & 999 & 1.3461 & +0.2872 & +0.4839 & +9.25 & 0.0010 & -- \\
 & $H_0^{\text{diag}}$ (VAR Diag) & 999 & 1.3461 & +0.0917 & +0.1612 & +34.32 & 0.0010 & -- \\
 & $H_0^{\text{static}}$ (Static Mode) & 9,999 & 1.3461 & +0.9877 & +1.2566 & +2.58 & 0.0184 & 0.0396 \\
 & $H_0^{\text{diag+contemp}}$ (Cross-lag Isol.) & 9,999 & 1.3461 & +0.8952 & +0.9724 & +9.74 & 0.0001 & 0.0006 \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{itemize}
    ...
    \item Overall historical concentration: 99.98\% of observations satisfy $q^* \le 4$ with modal $q^* = 2$ (accounting for 88.0\% of all windows).
    \item Cross-method dimensional concordance: analytical SVD emergence achieves 100.0\% agreement within $\pm 1$ dimension.
\end{itemize}
...
\end{itemize}
Because average correlation, effective rank, and connectedness exhibit substantial multicollinearity ($\text{VIF} > 13$), individual partial regression coefficients should not be interpreted as isolated economic channels. The complete linear specification accounts for 72.84\% of linear variation ($R^2 = 72.84\%$, leaving 27.16\% unexplained by this linear combination).
...
\begin{itemize}
    \item \textbf{2008 GFC Peak:} Mean $\mathrm{CEFI}_{\text{res}} = +0.0122$ (Median $= +0.0077$).
    \item \textbf{2020 COVID Shock:} Mean $\mathrm{CEFI}_{\text{res}} = +0.0504$ (Median $= +0.0786$).
    \item \textbf{2000 Dot-Com Crash:} Mean $\mathrm{CEFI}_{\text{res}} = -0.1009$ (Median $= -0.1134$).
    \item \textbf{2022 Rate Tightening:} Mean $\mathrm{CEFI}_{\text{res}} = +0.0538$ (Median $= +0.0112$).
\end{itemize}
...
\midrule
None (Full Sample)        & +0.189 & -0.345 & +0.534 & +5.01 & $5.41 \times 10^{-07}$ \\
Exclude Dot-Com Crash     & +0.189 & +0.020 & +0.169 & +1.25 & $2.10 \times 10^{-01}$ \\
Exclude 2008 GFC          & +0.198 & -0.345 & +0.542 & +4.56 & $5.15 \times 10^{-06}$ \\
Exclude 2020 COVID Shock  & +0.188 & -0.345 & +0.533 & +4.65 & $3.29 \times 10^{-06}$ \\
Exclude 2022 Rate Tightening & +0.189 & -0.484 & +0.673 & +7.88 & $3.33 \times 10^{-15}$ \\
\bottomrule
\end{tabular}
\end{table}

In all liquidity-exclusion cases, $\Delta \beta$ remains strictly positive and statistically significant (Wald $t \ge 4.56$, full sample $t = 5.01$). The contrast remains positive and statistically significant after omitting each liquidity shock individually.
